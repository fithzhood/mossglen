...
function newGame(seed) {
  ...
  DATA.villagers.forEach(function (v) {
    S.vill[v.id] = { met: false, stageSeen: 1, said: {}, idle: {}, wishIdx: 0, given: 0, wishDay: 0, queue: [] };
  });
  return S;
}
...
/* Hand every villager something to remark on next time you speak to them.
   Queued, not computed on demand, so a reaction cannot be overwritten by the
   next thing that happens before it has been delivered. */
function queueForAll(S, key, textFor) {
  DATA.villagers.forEach(function (v) {
    var st = S.vill[v.id];
    ...
    if (!text) return;
    st.said[key] = 1;
    st.queue.push({ key: key, text: text });
    /* a villager who owes you nine remarks is a villager nobody wants to talk
       to; keep the freshest few and let the rest go */
    if (st.queue.length > 3) {
      /* dropping it must also un-mark it, or the line is marked spoken and
         never spoken — which is exactly the unreachable-content failure the
         queue exists to prevent */
      var dropped = st.queue.shift();
      delete st.said[dropped.key];
    }
  });
}
...
function actGive(S, act, ev) {
  ...
  var again = st.given > v.wishes.length && DATA.thanksAgain[v.id] && DATA.thanksAgain[v.id][w];
  var key = v.id + (again ? '.thanksAgain.' : '.thanks.') + w;
  var text = again ? DATA.thanksAgain[v.id][w] : v.lines.thanks[w];
  S.seenLines[key] = 1;
  ev.push({ type: 'say', who: v.id, name: v.name, sprite: v.sprite, text: text, key: key });
  ev.push({ type: 'gave', who: v.id, item: w, bloom: S.bloom });
  if (S.stage > before) {
    for (var gn = before + 1; gn <= S.stage; gn++) {
      (function (m) {
        queueForAll(S, 'stage.' + m, function (vv) { return vv.lines.stage[m]; });
      })(gn);
    }
    ev.push({ type: 'stage', stage: S.stage, name: stg.name, note: stg.note });
  }
  return ev;
}
...
function actDonate(S, act, ev) {
  ...
  if (S.stage > before) {
    for (var n = before + 1; n <= S.stage; n++) {
      (function (m) {
        queueForAll(S, 'stage.' + m, function (v) { return v.lines.stage[m]; });
      })(n);
    }
    ev.push({ type: 'stage', stage: S.stage, name: st.name, note: st.note });
  }
  return ev;
}
...
function actPlace(S, act, ev) {
  ...
  queueForAll(S, 'home.' + act.item, function (v) { return v.lines.home[act.item]; });
  var mood = roomMood(S);
  if (mood) queueForAll(S, 'room.' + mood, function (v) { return DATA.roomMoods[mood][v.id]; });
...
function actTalk(S, act, ev) {
  ...
  /* whatever they have been waiting to say about something you did */
  if (!line && st.queue && st.queue.length) {
    var q = st.queue.shift();
    line = { key: v.id + '.' + q.key, text: q.text };
  }
  ...
  if (!line) {
    ...
    /* stride 3 against a pool of 7 still reaches every line before repeating
       any, but keeps three villagers from marching through their pools in
       lockstep the way a stride of 1 does */
    st.idle[tod] = (idx + 3) % pool.length;
  }
...
function allLineKeys() {
  ...
  DATA.villagers.forEach(function (v) {
    ...
    Object.keys(DATA.thanksAgain[v.id] || {}).forEach(function (i) {
      keys.push(v.id + '.thanksAgain.' + i);
    });
...
if (typeof document !== 'undefined') (function () {
  ...
  var walkTo = null, pending = null, bob = 0, pickingSlot = -1, ghostItem = null;
  var wander = {};
  ...
  function drawHome() {
    ...
    DATA.homeSlots.forEach(function (sl, i) {
      ...
      if (pickingSlot === i) {
        ...
        var a = (0.55 + 0.3 * Math.sin(t)).toFixed(2);
        /* a pool of light ON the surface, not a box floating above it */
        ctx.fillStyle = 'rgba(255,236,160,' + (a * 0.5).toFixed(2) + ')';
        ctx.beginPath();
        ctx.ellipse(px(sl.x), px(sl.y + 7), 11 * SCALE, 3.5 * SCALE, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = 'rgba(255,236,160,' + a + ')';
        ctx.lineWidth = Math.max(2, SCALE * 0.6);
        ctx.beginPath();
        ctx.ellipse(px(sl.x), px(sl.y + 7), 11 * SCALE, 3.5 * SCALE, 0, 0, Math.PI * 2);
        ctx.stroke();
        /* and the thing you are about to put down, shown where it would go */
        if (ghostItem) {
          ctx.globalAlpha = 0.45 + 0.2 * Math.sin(t);
          blit('item_' + ghostItem, sl.x - 6, sl.y - 6);
          ctx.globalAlpha = 1;
        }
  ...
  function drawMat() {
    ...
    ctx.fillStyle = '#a0763f';
    ctx.fillRect(px(144), px(202), px(34), px(14));
    ctx.fillStyle = '#c09155';
    var m;
    for (m = 146; m < 178; m += 4) ctx.fillRect(px(m), px(202), SCALE * 2, px(14));
    ctx.fillStyle = 'rgba(94,62,34,0.45)';
    for (m = 204; m < 216; m += 4) ctx.fillRect(px(144), px(m), px(34), SCALE);
    /* fringe, so it reads as a mat and not a painted rectangle */
    ctx.fillStyle = '#c9a267';
    for (m = 145; m < 179; m += 3) {
      ctx.fillRect(px(m), px(198), SCALE, px(2));
      ctx.fillRect(px(m), px(216), SCALE, px(2));
    }
  }
  ...
  function slotTapped(i) {
    ...
    if (!held.length) {
      ...
      held.forEach(function (it) {
        ...
        b.appendChild(n);
        if (S.inv[it] > 1) { var c = document.createElement('em'); c.textContent = S.inv[it]; b.appendChild(c); }
        b.onmouseenter = b.onfocus = function () { ghostItem = it; };
        b.ontouchstart = function () { ghostItem = it; };
